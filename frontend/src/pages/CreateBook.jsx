...
const CreateBook = () => {
  ...
  const handleSaveBook = () => {
    ...
    axios
      ...
      .then(() => {
        setLoading(false);
        enqueueSnackbar("Book is successfully created.", { variant: 'success' });
        navigateURL('/');
      })
      .catch((err) => {
        setLoading(false);
        enqueueSnackbar("An error occurred while saving the book. Check the console.", { variant: 'error' });
        console.log(err);        
      });
  };
  ...
  return (
    <div className='p-5'>
      ...
      <div className='flex flex-col border-2 bg-gray-800 border-gray-800 rounded-md w-full max-w-[600px] p-4 mx-auto'>
        ...
        <button className='p-2 bg-sky-300 hover:bg-sky-400 hover:font-bold m-8 rounded-lg' onClick={handleSaveBook}>
          Save Book
        </button>
      </div>
    </div>
