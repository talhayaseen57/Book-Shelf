...
const UpdateBook = () => {
  ...
  const handleEditBookFunc = () => {
    ...
    axios
      ...
      .then(() => {
        setLoading(false);
        enqueueSnackbar("The book is successfully updated.", { variant: 'success' });
        navigateURL('/');
      })
      .catch((err) => {
        setLoading(false);
        enqueueSnackbar("An error occurred while updating the book. Check the console.", { variant: 'error' });
        console.log(err);        
      });
  };
